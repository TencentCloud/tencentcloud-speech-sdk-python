...
# ---------------------------------------------------------------------------
class RealtimeRecognitionListenerV2:
    """
...
# ---------------------------------------------------------------------------
# RealtimeRecognizerV2  –  realtime v2 sentence-mode ASR
# 首版功能与 speaker 版等价：句子模式实时识别，话者分离作为可选开关（默认关闭）。
# ---------------------------------------------------------------------------
class RealtimeRecognizerV2:
    ...
    def __init__(self, appid, credential, engine_model_type, listener):
        ...
        # request params (defaults aligned with Go V2 SDK)
        self.voice_format = 1
        self.need_vad = 1
        ...
        # speaker context params（话者分离相关能力，默认关闭）
        ...
        # 扩展参数：用于支持隐藏参数或后续新增参数，无需升级 SDK。
        # 这里的 key/value 会以 query string 形式拼接到 ws URL 上，并参与签名计算。
        # 同名 key 会覆盖 SDK 默认填充的字段（如 voice_format、needvad 等），
        # 这是为隐藏参数的"覆盖默认值"场景刻意保留的能力。
        self.extra_params = {}
    ...
    def set_extra_param(self, key, value):
        """设置单个扩展参数，同名 key 会覆盖 SDK 默认填充的字段。"""
        if not key:
            return
        self.extra_params[key] = value

    def set_extra_params(self, params):
        """批量设置扩展参数，会与已有的 extra_params 合并（同名覆盖）。"""
        if not params:
            return
    ...
    def _create_query_arr(self):
        ...
        # 合并扩展参数：放在最后，允许覆盖 SDK 默认填充的字段；
        # 空 key 直接忽略，避免污染 query 串。
    ...
    def start(self):
        ...
        logger.info("%s realtime v2 recognition start" % self.voice_id)
        ...
        try:
            while True:
                ...
                if msg.get('final', 0) == 1:
                    ...
                    logger.info("%s realtime v2 recognition complete" % self.voice_id)
                    break
